...
logger = logging.getLogger(
            "owldyn.vidlinks.library.ffmpeg")
class Ffmpeg:
    ...
    def get_information(self, file: tempfile.NamedTemporaryFile) -> dict:
        ...
        if self._information is None:
            ...
            json_string = self.run_ffprobe_command_on_file(
                self.Commands.FFPROBE, args)[0].decode('utf-8')
            try:
    ...
    def run_ffprobe_command_on_file(self, command: Commands,
                                   args: list) -> tuple:
        """Runs the command with the given args on the given file.
    ...
    @staticmethod
    def _replace_file(original_file: tempfile.NamedTemporaryFile, new_file: tempfile.NamedTemporaryFile):
        new_file.seek(0)
    ...
    def replace_audio(self, file: tempfile.NamedTemporaryFile):
        ...
        self.run_ffmpeg_command_on_file(
            self.Commands.FFMPEG, args, file)
