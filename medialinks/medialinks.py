...
class MediaLinks(commands.Cog):
    ...
    async def add_api(self, ctx, service, *settings):
        ...
        if not api_options:
            await ctx.send(
                f"{service} not found in supported apis. Supported apis: {list(self.supported_apis.keys())}"
            )
            return
        ...
        for setting in settings:
            ...
            if setting_split[0] not in api_options:
                await ctx.send(
                    f"{setting_split[0]} is not a valid setting for {service}."
                )
                return
    ...
    async def _automedialinkignore_server(self, ctx):
        ...
        if guild.id in guilds:
            ...
            await ctx.send(
                "I will ignore this server. Explicitly use !redditlink to use this feature."
            )
    ...
    async def _automedialinkignore_channel(self, ctx):
        ...
        if chan.id in chans:
            ...
            await ctx.send(
                "I will ignore this channel. Explicitly use !redditlink to use this feature."
            )
    ...
    async def _automedialinkignore_user(self, ctx):
        ...
        if user.id in users:
            ...
            await ctx.send(
                "I will ignore your links. Explicitly use !redditlink to use this feature."
            )
    ...
    async def automedialink(self, message):
        ...
        exclusions = [
            ...
            bool(message.guild)
            and bool(message.guild.id in await self.conf.guilds_ignored()),
            bool(message.channel)
            and bool(message.channel.id in await self.conf.channels_ignored()),
            bool(message.author)
            and bool(message.author.id in await self.conf.users_ignored()),
        ]
                    ...
                    for index, message in enumerate(messages_to_send):
                        ...
                        if spoiler and message.type == message.MessageTypes.IMAGE_EMBED:
                            if not message.image_url:  # Can't do the hack here
                                await sender(
                                    **message.send_kwargs, mention_author=False
                                )
                            else:
                                spoiler_setup = await sender(
                                    f"||{message.image_url}||", mention_author=False
                                )
                                await spoiler_setup.edit(**message.send_kwargs)
                        else:
                            file = None
                            if (
                                message.send_kwargs.get("file")
                                and message.send_kwargs.get("embed")
                                and message.type == message.MessageTypes.VIDEO
                            ):
