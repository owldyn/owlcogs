...
class Timestamp(commands.Cog):
    ...
    async def print_timezone(self, message: discord.Message):
        if "<" in message.content and ">" in message.content:
            ...
            if match:
                try:
                    parsed_time = parse(match.group(1))

                    ctx: commands.Context = await self.bot.get_context(message)
                    try:
                        timezone = await self.conf.user(message.author).get_raw("timezone")
                    except Exception:
                        await ctx.reply(
                        ...
                        return
                    time = int(
                        parsed_time.timestamp() - parsed_time.astimezone(ZoneInfo(timezone)).utcoffset().total_seconds()
                    )
